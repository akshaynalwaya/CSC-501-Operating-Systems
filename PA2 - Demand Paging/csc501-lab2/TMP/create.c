...
{
	int j = 0,frame_number = 0;	
	pd_t *pdir_entry;

	get_frm(&frame_number);

	proctab[pid].pdbr = (frame_number + FRAME0) * NBPG;
	frm_tab[frame_number].fr_pid = pid;
	frm_tab[frame_number].fr_type = FR_DIR;
	frm_tab[frame_number].fr_status = FRM_MAPPED;
	
	pdir_entry = (FRAME0+ frame_number) * NBPG;
	
	while(j < 1024){
		pdir_entry[j].pd_write = 1;
		
		switch(j){
			case 0:
				pdir_entry[0].pd_pres = 1;
				pdir_entry[0].pd_base = FRAME0 + 0;
				break;
			case 1:
				pdir_entry[1].pd_pres = 1;
				pdir_entry[1].pd_base = FRAME0 + 1;
				break;
			case 2:
				pdir_entry[2].pd_pres = 1;
				pdir_entry[2].pd_base = FRAME0 + 2;
				break;
			case 3:
				pdir_entry[3].pd_pres = 1;
				pdir_entry[3].pd_base = FRAME0 + 3;
				break;
			default:
				break;
		}
